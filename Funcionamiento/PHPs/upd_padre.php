...
//Archivos que requerimos
    ...
    if(isset($_POST['actualizar']))
    {
        ...
        { 
            ...
            if(strcmp($nombre,$NomPad) !== 0)
            {
                //Query para actualizar la información en la BDD
                $actualizar = "UPDATE padre SET Nombre = '$nombre' WHERE Username = '$user' ";
...
/*Comparamos lo que tiene el campo de texto con el valor recogido de la base de datos,
  si es diferente, procedemos a actualizar la información, en caso contrario no se actualiza nada*/
            ...
            {
                $actualizar = "UPDATE padre SET Ape_Pat = '$ap_pat' WHERE Username = '$user' ";
                if(!$conexion->query($actualizar) === TRUE)
                {
       ...
       $ap_mat = validar($_POST['ap_mat']);
        ...
        else
        {
            ...
            {
                $actualizar = "UPDATE padre SET Ape_Mat = '$ap_mat' WHERE Username = '$user' ";
                if(!$conexion->query($actualizar) === TRUE)
                {
       ...
       $tel = validar($_POST['tel']);
        ...
        {
            $actualizar = "UPDATE padre SET Tel = $tel WHERE Username = '$user' ";    
            if(!$conexion->query($actualizar) === TRUE)
            {
        ...
        else
        {
            ...
            {
                $actualizar = "UPDATE padre SET Correo = '$correo' WHERE Username = '$user' ";
                if(!$conexion->query($actualizar) === TRUE)
                {
       ...
       $user2 = validar($_POST['usuario']);
        ...
        else
        {
            ...
            {
                $actualizar = "UPDATE padre SET Username = '$user2' WHERE Username = '$user' ";
                if(!$conexion->query($actualizar) === TRUE)
                {
        ...
        elseif(count($fecha) == 3 && checkdate($fecha[1], $fecha[0], $fecha[2]))
        {
            ...
            $actualizar = "UPDATE padre SET Keyword = '$fecha2' WHERE Username = '$user' ";     
            if(!$conexion->query($actualizar) === TRUE)
            {
