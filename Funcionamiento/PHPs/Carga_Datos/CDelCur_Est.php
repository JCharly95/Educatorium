<?php
    ...
    $SelGrado=$CurNam=$ID_Cur=$ID_Estu="";
    ...
    {
        $ID_Estu=$res['ID_Estudiante'];
    }

    $sql1="select ID_Curso, Curso.Nombre as CurNam from Curso_Estudiante inner join Curso on (Curso_ID=ID_Curso)";
    $sql2=" inner join Estudiante on (Estudiante_ID=ID_Estudiante) where Username='".$User."';";
    $sql=$sql1.$sql2;
    $consulta=$conexion->query($sql);
    while($res=$consulta->fetch_assoc())
    {
        $Cursos[]=array($res['ID_Curso'],$res['CurNam']);
    }

    if(isset($_POST['enviar']))
    {
        $ID_Cur=$_POST['CurSelDel'];

        //Borrar la relacion del curso estudiante
        $sql="delete from Curso_Estudiante where Estudiante_ID=".$ID_Estu." and Curso_ID=".$ID_Cur.";";
        $consulta=$conexion->query($sql);
        echo '<script>alert("Te has salido del curso seleccionado");</script>';
        echo '<META HTTP-EQUIV="REFRESH" CONTENT="1;URL=Principal_Est.php">';
    }
