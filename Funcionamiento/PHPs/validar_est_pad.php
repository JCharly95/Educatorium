...
//DECLARAMOS LAS VARIABLES DEL FORM Y LAS VARIABLES DE ERROR COMO VACIAS
    $nombre=$ap_pat=$ap_mat=$tel=$correo=$user=$pass=$cpass=$esc_sel=$grado=$Img=$clave=$ID_Tip_Rec=$ID_Tip_Key="";
...
//SI TODO ESTA CORRECTO PROCEDEMOS A REGISTRAR AL USUARIO
        if(!empty($nom_right) && !empty($pat_right) && !empty($mat_right) && !empty($tel_right) && !empty($cor_right) && !empty($us_right) && !empty($pas_right) && !empty($cpa_right) && !empty($esc_rel_right) && !empty($grado_right) && !empty($key_right))
        {
            ...
            if($result == 0)
            {
                ...
                $ins1="insert into Estudiante (Nombre, Ape_Pat, Ape_Mat, Correo, Tel, Username, Password, Keyword, Tip_Key_ID, Grado_ID, Apoyo_ID, Escuela_ID) values ('";
                $ins2=$nombre."','".$ap_pat."','".$ap_mat."','".$correo."',".$tel.",'".$user."','".$cifrado."','".$Keyword."',".$ID_Tip_Rec.",".$grado.",".$ID_Img.",".$esc_sel.");";
                $ins=$ins1.$ins2;
